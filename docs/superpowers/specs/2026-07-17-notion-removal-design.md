...
- `adminUpdate` принимает поля переводов свободного текста: `fullName_en/pt`,
  `quote_en/pt` (колонки уже в модели — их читает лендинг).
- `POST /teacher-anketa/:id/translate` (ADMIN/OWNER) — автоперевод-черновик.
  **ОТЛОЖЕНО в самый конец дорожной карты (решение владельца 2026-07-24).**
  Эндпоинт реализован и вмёржен, но СПИТ: кнопка в админке убрана, без
  `ANTHROPIC_API_KEY` в env бота недостижим из UI. Модель — `claude-sonnet-5`
  (для перевода коротких текстов Opus избыточен; допустим и Haiku). Один вызов
  Claude API, structured outputs — ответ строго `{fullName_en, fullName_pt,
  quote_en, quote_pt}`; в промпте: бразильский португальский, имена
  транслитерировать; заполняет только пустые поля. НЕ сохраняется автоматически —
  сохранение через `admin-update` после вычитки человеком. Включение — вернуть
  кнопку в админку и задать `ANTHROPIC_API_KEY`.
...
- `AnketsList.jsx`: бейдж статуса (draft/pending/approved), кнопки
  ...
  `fullName_en/pt`, `quote_en/pt` (заполняются ВРУЧНУЮ). Кнопка «Перевести
  автоматически» убрана (автоперевод отложен, см. выше); при включении
  вернётся хендлер, дёргающий `/translate` и заполняющий только пустые поля.
  Лендинг фолбэкает на ru, поэтому переводы заполняются постепенно.
...
- Теги — без API: специализации и интересы выбираются в онбординге из
  фиксированных списков, поэтому переводится словарь, а не значения — модуль
  `lib/tagTranslations.ts` (ru → en/pt; словарь снимается с констант онбординга
  tucan). `getTutors()` мапит теги по словарю с фолбэком на ru; колонки
  `specializations_en/pt`, `interests_en/pt` больше не читаются. LLM-перевод
  остаётся только для свободного текста (имя, цитата).
...
- Миграция: `DROP COLUMN "notionPageId"` (колонки `gender/age/timezone/
  ...
  После переезда тегов на словарь (Фаза 1) также дропнуть
  `specializations_en/pt`, `interests_en/pt` — из таблицы и из модели бота.
...
5. **Фаза 5 (в самый конец): включить автоперевод анкет** — вернуть кнопку
   «Перевести автоматически» в AnketsList (хендлер на спящий `/translate`),
   задать `ANTHROPIC_API_KEY` в env бота. Модель `claude-sonnet-5` (или Haiku).
   Инфраструктура (эндпоинт, схема, поля) уже вмёржена в Фазе 1.

## Вне рамок (YAGNI)

- Просмотр/управление лидами в админке tucan (лиды сохраняются в Postgres; UI — позже).
- Автоперевод CMS-контента лендинга (тексты секций переводятся вручную;
  автоперевод-черновик есть только у анкет — Фаза 1).
- Миграция исторических лидов из Notion — не нужна: существующие лиды были
  тестовыми (решение владельца 2026-07-17), Notion-база лидов просто
  выбрасывается.
...
- **Фаза 1:** draft-анкета не видна на лендинге и в выборе преподавателя;
  ...
  вписанные вручную в админке, видны на en/pt-лендинге; теги на en/pt-лендинге
  переведены словарём. (Автоперевод отложен — не проверяется в Фазе 1.)
