...
- `POST /teacher-anketa/:id/translate` (ADMIN/OWNER) — автоперевод-черновик
  (гибрид, утверждён 2026-07-17). Один вызов Claude API (модель
  `claude-opus-4-8`, structured outputs — ответ строго в форме
  `{fullName_en, fullName_pt, quote_en, quote_pt}`; в промпте: бразильский
  португальский, имена транслитерировать) для пустых полей переводов. Результат
  возвращается админке и НЕ сохраняется автоматически — сохранение только через
  `admin-update` после вычитки человеком. Требует `ANTHROPIC_API_KEY` в env
  бота; цена ~3–4 цента за анкету.
...
- `AnketsList.jsx`: бейдж статуса (draft/pending/approved), кнопки
  ...
  `fullName_en/pt`, `quote_en/pt` и кнопка «Перевести автоматически» (дёргает
  `/translate`, подставляет черновик в поля; админ вычитывает и сохраняет).
  Лендинг фолбэкает на ru, поэтому переводы заполняются постепенно.
  Устаревание переводов: учитель правит русскую анкету → админам уже приходит
  TG-уведомление с ❗-маркерами изменённых полей → админ повторно жмёт
  «Перевести».
...
- **Фаза 1:** draft-анкета не видна на лендинге и в выборе преподавателя;
  ...
  из админки видны на en/pt-лендинге; кнопка «Перевести» возвращает черновик
  en/pt (pt — бразильский), ничего не сохраняя сама; теги на en/pt-лендинге
  переведены словарём.
