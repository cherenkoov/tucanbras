...
`?duowk=1` / `?duoblur=1` (WebKit-путь дуотона на любом движке / попытка тонировать его
через blur-барьер), `/wk-probe.html` — пробник возможностей backdrop-filter на устройстве.
...
> фона. Гейт палитры снят: на WebKit дуотон деградирует до почти-белый/почти-чёрный, но
> НЕ теряет живой сэмпл — точное чтение правильного места важнее бренд-цвета из
> неправильного. Chromium/Gecko-телефоны получают точную палитру (они и не были
> проблемным движком).
...
- **WebKit** (iOS + десктопная Safari; рендерит url(#) в backdrop-filter как НИЧТО): встроенная цепочка
  ...
  `brightness`, `invert` даёт тёмный фон→светлый / светлый→тёмный). Сама цепочка даёт
  только почти-белый/почти-чёрный: brightness/contrast/invert действуют одинаково на все
  каналы, оба полюса делят один тон, палитру зелёный↔синий выразить нечем.
  **Брендовый цвет на WebKit недостижим — это измерено, а не предположено (2026-08-09,
  живой iPhone, страница-пробник `public/wk-probe.html`).** Три двери, все закрыты:
  > 1. **`url(#)` внутри `backdrop-filter`** — WebKit парсит и рисует НИЧЕГО. Ради этого
  >    и существует встроенная цепочка.
  > 2. **Собственный `filter: url(#…)` на оверлее** — пересобрать бинарную пару вторым
  >    проходом. В Chromium работает (backdrop-образ реально уходит в `filter` элемента),
  >    на iPhone — нет, заголовки остались ч/б. Второй сэмпл через blend-режим тоже
  >    закрыт: элемент с backdrop-filter изолирует блендинг, а два слоя backdrop-filter
  >    друг над другом дают верхнему сэмпл НИЖНЕГО, а не страницы.
  > 3. **Тон внутри самой цепочки** — и вот тут корень. **WebKit НЕ ЗАЖИМАЕТ значения
  >    между функциями фильтра**, Chromium зажимает. `contrast(50)` даёт не 0 и 1, а
  >    примерно −22 и +8.7; они линейно проходят всю цепочку и обрезаются только в САМОМ
  >    конце. Пробник поймал это в лоб: `contrast(0.16)` отдельно сводит две половины в
  >    серый, а он же ПОСЛЕ `contrast(50)` не делает ничего — остаются чистые белый и
  >    чёрный. Следствие: при единственном зажиме в конце каждый канал либо насыщается в
  >    0/1, либо становится константой, не зависящей от фона, — достижимы только УГЛЫ
  >    RGB-куба. Ни `#8fd096`/`#2e67b2`, ни пара зелёный↔зелёный-тёмный углами не
  >    являются.

  Тонированная цепочка недолго была дефолтом и **откачена**: на iPhone она красила белым
  и чёрным везде, кроме тонкой каймы, где яркость фона попадает в ~2% от порога 0.70 (там
  `contrast(50)` не успевает улететь за диапазон) — заголовки показывали ТРИ цвета вместо
  двух, третьим был тёмно-зелёный шов. Шов был багом, а не целью.
  ...
  `?duoblur=1` — **открытый эксперимент**: `blur(0.5px)` перед тоном в надежде, что
  растеризация сработает как барьер-зажим и оживит тонирование. Подтвердить или похоронить
  можно только на устройстве. Фиттер тона — `npm run fit:duotone-chain` (заодно показывает,
  что зелёный↔синий не берётся ни при каких параметрах: вместо зелёного полюса выходит
  голубой `#63a7d9`, промах 90/255).
...
Единица `--plan-u` в globals.css: процент высоты контейнера на десктопе, ширины на
мобилке (у кнопки всегда высота). Клип — маска силуэта плашки, те же значения, что на
frost-слое. Тона задаёт инстанс, не файл: перекрашенные копии делает
`npm run gen:plan-decor-art` (из пары тонов заливкой является ТЁМНЫЙ, светлый даёт
внутренний слой screen). Гард — `npm run verify:plan-decor` (+ `-- <url> 390x844`).
**В таблице 10 инстансов из 16.** Не вошли: четыре `Flower 3 - Become` (тариф 3) — фит не
берёт порог ни на одном угле, лучшее IoU 0.372, разбираться отдельно; и два
`Flower 2 - Cover` (кнопка тарифа 0) — ждут референса `3483:45351` из Figma MCP
(у прежнего окно 119×99, композит закрывал его целиком, IoU 0.415 у любого угла).
Мобильная композиция — перенос, а не макет: мобильного фрейма Plans с декором нет.
