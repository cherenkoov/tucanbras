...
1. **Адаптивный текст на мобилке не доделан.** Статичная заливка выбирает
   источник по ПОЗИЦИИ (y) и порядку отрисовки слоёв, а не по тому, что реально
   видно ПОД текстом (z): текст может считать фон, который перекрыт другим слоем
   (карточкой, спрайтом). На десктопе backdrop-filter сэмплирует настоящие
   пиксели, на таче он не рендерится (iOS парсит, но не рисует url(#) —
   проверено на устройстве). Возможные направления: растеризация композита в
   canvas и сэмплирование из него; ручные override-источники для проблемных
   заголовков.
2. **Плавность параллакса на мобилке** + в stack-секциях (CELPE-BRAS, Plans) фон
   дёргается вверх-вниз при скролле; иногда stack начинается не с первого
   элемента.
3. **Фантомные прыжки по якорям при тапах** в местах без кнопок: тап по input в
   FooterForm может перекинуть к hero или CELPE-BRAS; тап между hero и about —
   к Plans. Подозрение на невидимые/растянутые интерактивные зоны или
   focus-scroll. Диагностировать.
4. **Пайплайн учителей: БД ↔ лендинг ↔ Notion (admin).** Учителя приходят через
   приложение → онбординг → анкета → одобрение админа → после этого появляются
   на лендинге и в Notion. Продумать синхронизацию (кросс-репо: tucan-bot +
   landing + Notion).
5. **Ловушка скролла в карусели Tutors (мобилка):** после горизонтального свайпа
   карусели вертикальный свайп по карточкам перестаёт скроллить страницу —
   выбраться можно только тапом вне карточки. Правильный фикс — направленный
   touch-обработчик карусели: preventDefault/захват жеста только когда
   |dx| > |dy|, вертикальные свайпы отдавать странице (`touch-action: pan-x` /
   direction lock). Дополнительно рассматривались: scrollbar у карусели;
   кнопка-стрелка вниз после пролистывания карточек (можно добавить как
   affordance, но корень — в захвате жеста).
6. **Stack-карточки Plans прижаты к header** — центрировать по y относительно
   viewport, как сделано в CELPE-BRAS stack.
7. **Редизайн FooterForm.**
8. **Зонтики растягиваются мобильным scaleY(1.2).** Фон на телефонах тянется по
...
9. **Акцентный шрифт: добавить недостающие глифы** для португальского
   (ã, õ, ç, á, é, í, ó, ú, â, ê, ô…) — сейчас отображается некорректно.
10. **Легальные страницы:** Пользовательское соглашение, Политика
