...
только плашки: внутри кнопок мобильный макет декора не несёт вовсе. Пересчитать десктопные
числа было НЕЛЬЗЯ — у `3499:46325` другой угол (−64.04 против −115.96) и нет флипа, то
есть вышел бы просто не тот лист. Оба набора в разметке, выбор брейкпоинтом (`lg:hidden` /
`hidden lg:block`), как у самой плашки.
