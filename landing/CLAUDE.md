...
Адаптивный текст (заголовки, реагирующие на фон дуотоном ink↔cream по яркости 0.70) —
...
основной путь.
- **Десктоп** (движки, парсящие url(#) reference-filter): `backdrop-filter: url(#adaptive-duotone)`
  — точный ink/cream, отражает движущиеся спрайты.
- **Тач/iOS** (WebKit рендерит url(#) в backdrop-filter как НИЧТО): встроенная цепочка
  ...
  `brightness`, `invert` даёт тёмный фон→cream / светлый→ink; цвета почти-белый/почти-чёрный).
...
- **Фолбэк на СТАРУЮ static-fill** (стек CSS background-слоёв под глифами: карточки
  ...
  на iOS не рендерится → ручной `staticFill="ink"|"cream"`, применён: VS в Comparison).
...
4. **Ловушка скролла в карусели Tutors (мобилка):** после горизонтального свайпа
