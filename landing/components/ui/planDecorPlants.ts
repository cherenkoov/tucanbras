...
export const PLAN_DECOR: readonly Record<PlanDecorSlot, readonly PlanDecorPlant[]>[] = [
  {
    ...
    /*
      Кнопка «Single lesson» декорируется НЕ отсюда. У неё своя композиция прямо в
      `PlanSectionShared.tsx` (`TRY_FLOWERS`, узел 3483:45258): три полных цветка поверх
      плиты, с запечённой подложкой акцента под multiply-тени, обрезаются живым
      `overflow` кнопки. Она пришла с main и остаётся ведущей — здешние срезы
      (`p0-btn-0/1`, файлы `tier1-btn-*.svg`) сняты, иначе на кнопке рисовались бы обе
      композиции разом.
      Для истории: фит для этой кнопки не работал в принципе — окна инстансов 260×99 и
      119×99, композит закрывает такую полоску целиком, IoU держался на 0.415 у ЛЮБОГО
      кандидата подряд, и оба инстанса стояли через BELOW_PASS_OK. То есть срезы тут были
      не капризом, а единственным способом; просто композиция с main — своя, независимая.
    */
    button: [],
  },
  {
