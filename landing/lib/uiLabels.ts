...
export interface UiLabels {
  ...
  /**
   * Надпись на hero-CTA. Живёт здесь, а не в Notion (где всё ещё лежит `hero.ctaText`
   * «Бесплатный урок»): кнопка теперь выбирает пробный урок как тариф, и её текст —
   * часть той же тройки состояний, что `planSelected` и `unselected`. Тот же случай,
   * что «Туторы» в `tutorsNav`, — строку владеет код, а не CMS.
   */
  tryFree:      string
  yourTutor:    string
  yourPlan:     string
