...
//
// ── AND THE DEVICE SAYS NO CHAIN EVER WILL (measured 2026-08-17, /duostack-probe.html) ──
// The next thing tried was moving the clamp off `blur()` and onto element composites —
// splitting the same stages across stacked sibling layers, since compositing clamps by
// definition. On the owner's iPhone the probe reported:
//   row B  invert∘invert across two siblings → the page comes back  ⇒ layers DO chain
//   row F  same with a mask on each layer    → palette inside the mask ⇒ masks are fine
//   row C  binarise, then contrast(0.538)    → near-white/near-black, NOT the two greys
//   row D  full palette stacked              → #5cffff and black — the production pair
// So the composite does NOT restore [0,1] there either, and rows C and D do not even agree
// on which end it clips: D matches "clips >1, negatives ride" at the first boundary and
// "no clip" at the second. Mixed, per-boundary, undetectable.
// That is fatal for the whole family, not just for one string. A hard threshold IS a clamp
// — every other built-in is affine, and a composition of affine maps cannot produce a step
// — so any binarise must lean on clamping somewhere. Refitting helps only half way: a
// THREE-stage tint with no out-of-range value anywhere hits both palette colours to 0.0/255
// (so the tint needs no clamp at all), but the binarise still does, and with per-boundary
// semantics unknown the PARITY of the pinning inverts — a brute force over every
// combination for 2-, 3- and 4-layer prefixes returns colours swapped (#2e67b2/#8fd096) or
// garbage (#ffff00) in a third of them. There is no arrangement that is right in all cases.
// Conclusion: on WebKit the duotone comes from the SVG filter on the element (which
// thresholds honestly, feComponentTransfer type="discrete", no clamp games) and the work
// belongs in making what the glyphs are filled WITH accurate. /duostack-probe.html stays as
// the retest for a future iOS.
//
