...
export default function FooterTucan({
  className = 'w-[80px] h-[79px] shrink-0',
  layer = 'head',
}: {
  className?: string
  layer?: Layer
  ...
  useEffect(() => {
    ...
    const blink = () => {
      ...
      if (upper && lower) {
        upper.style.animation = `tucan-lid-upper-blink ${BLINK_DURATION}ms linear forwards`
        lower.style.animation = `tucan-lid-lower-blink ${BLINK_DURATION}ms linear forwards`
        setTimeout(() => {
          if (upperRef.current) upperRef.current.style.animation = 'none'
          if (lowerRef.current) lowerRef.current.style.animation = 'none'
        }, BLINK_DURATION)
      }
      timerId = setTimeout(blink, MIN_INTERVAL + Math.random() * (MAX_INTERVAL - MIN_INTERVAL))
    }

    timerId = setTimeout(blink, Math.random() * MAX_INTERVAL)
      ...
      >
        ...
        {layer === 'head' && (
          <>
            ...
            {/* Eyelids — above Eye layers, below Head/Upper Beak Outline */}
            <g clipPath="url(#tucan-ft-eye-clip)">
              <g ref={upperRef} style={{ transform: 'translateY(-105%)', transformBox: 'fill-box' }}>
                <path d="M41.3 70.9 A 6.1 9.8 0 0 1 53.5 70.9 Z" fill="#D55C27"/>
              </g>
            </g>
            <g clipPath="url(#tucan-ft-eye-clip)">
              <g ref={lowerRef} style={{ transform: 'translateY(105%)', transformBox: 'fill-box' }}>
